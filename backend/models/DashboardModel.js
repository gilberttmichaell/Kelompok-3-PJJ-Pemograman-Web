...
const getSummary = async () => {
    ...
    const [[leadByStatus]] = await db.query (`SELECT status, COUNT(*) AS total FROM leads GROUP BY status`);
    const [[dealByStatus]] = await db.query (`SELECT stage, COUNT(*) AS total FROM deals GROUP BY stage`);
